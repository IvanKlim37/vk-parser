...
def load_env():
    """Читает .env файл если он есть, иначе берёт из os.environ."""
    env = {}
...
_env = load_env()
VK_TOKEN  = _env.get("VK_TOKEN")  or os.environ.get("VK_TOKEN",  "")
...
def bot_polling():
    ...
    marker = None
    print("Бот запущен. Ожидаю /start в MAX...")
    ...
    while True:
        try:
            ...
            for upd in updates:
                ...
                if not chat_id:
                    chat_id = msg.get("sender", {}).get("user_id")
                if not chat_id:
                    continue
                ...
                if text in ("/start", "start", "старт"):
                    ...
                    save_subscribers(subscribed_chats)
                    max_send(chat_id, "Вы отписаны. Напишите /start чтобы подписаться снова.")
                    print(f"  [-] Отписка: {chat_id}")

                elif text in ("/now", "now", "сейчас"):
                    # Немедленный запуск для одного пользователя
                    max_send(chat_id, "Собираю посты, подождите...")
                    threading.Thread(
    ...
    for attempt in range(3):
        try:
            ...
            if "error" in data:
                ...
                if code in (5, 1116):
                    print(f"  [!!!] ТОКЕН VK НЕДЕЙСТВИТЕЛЕН (код {code}). Замените VK_TOKEN.")
                    return []
                if code == 6:
                    time.sleep(2)
                    continue
...
def collect_urls(window_from: datetime, window_to: datetime) -> list:
    """Собирает подходящие ссылки за указанный период."""
    urls = []
...
def run_parser_for(chat_ids: list, window_from=None, window_to=None):
    """Запускает парсер и отправляет результат указанным chat_id."""
    now = now_msk()
    ...
    for chat_id in chat_ids:
        ...
        print(f"  → Отправлено {chat_id}: {len(urls)} ссылок")


def run_parser():
    """Плановый запуск каждые 2 часа — рассылает всем подписчикам."""
...
if __name__ == "__main__":
    if VK_TOKEN == "вставьте_вк_токен_сюда":
        print("[!] Вставьте VK_TOKEN в скрипт")
        exit(1)
    if MAX_TOKEN == "вставьте_макс_токен_сюда":
        print("[!] Вставьте MAX_TOKEN в скрипт")
        exit(1)

    print("VK Парсер → MAX бот · Магнитогорск")
    print(f"Подписчиков загружено: {len(subscribed_chats)}")
    print("Напишите /start боту в MAX\n")
